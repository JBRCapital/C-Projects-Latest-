...
namespace UpdateSalesforceData
{
    class IntroducerUpdater
    {
        ...
        {
            SalesforceDataHelper.SalesforceClient.BulkUpsertFromSQL("Introducers", "Account", "sentinalCompanyId__c",
            ...
            LogHelper.Logger.WriteOutput(string.Concat("Ended Sync of Introducers"), Program.EmailTransactionLog);
        }


        private static SqlDataReader GetAccountDataFromSQL()
        {
            return new SqlCommand("SELECT * FROM SQL_SF_Sync_Introducers() Order By Number", SQLDataConnectionHelper.SqlConnection) { CommandTimeout = 0 }.ExecuteReader();
        }
