...
namespace UpdateSalesforceData
{
    class ProposalUpdater
    {
        ...
        public static void UpdateProposalData(SqlConnection sqlConn, SalesforceHttpClient salesforceClient)
        {
            Console.WriteLine(string.Concat("Started Sync of Proposals"));

            var proposalData = GetProposalDataFromSQL(sqlConn);
            var dateTimeSyncStarted = DateTime.Now;

            while (proposalData.Read())
            {
                var proposalDataForSF = new ProposalData()
                {
                    sentinalID__c = proposalData["ProposalId"].ToString(),

                    dateTimeProposalFirstAccepted__c = DataHelper.GetDateTime(proposalData["FirstDateTimeAccepted"]),
                    firstAcceptedStatus__c = proposalData["FirstAcceptedStatus"].ToString(),
                    ...
                    firstDecisionedStatus__c = proposalData["FirstDecisionedStatus"].ToString(),

                    timeTakenIn_NewProposals__c = DataHelper.GetDouble(proposalData["Active Proposals"]) + DataHelper.GetDouble(proposalData["New Proposals"]),
                    ...
                    timeTakenIn_TotalCredit__c = DataHelper.GetDouble(proposalData["Credit Manager"]) + DataHelper.GetDouble(proposalData["Credit Committee"]) + DataHelper.GetDouble(proposalData["Funder"]),
                    timeTakenIn_CreditPlusReferred__c = DataHelper.GetDouble(proposalData["Referred for Information"]) + DataHelper.GetDouble(proposalData["Active Proposals"]) + DataHelper.GetDouble(proposalData["New Proposals"]),
                    timeTakenIn_CreditPlusSalesSupport__c = DataHelper.GetDouble(proposalData["Active Proposals"]) + DataHelper.GetDouble(proposalData["New Proposals"]) + DataHelper.GetDouble(proposalData["Credit Manager"]) + DataHelper.GetDouble(proposalData["Credit Committee"]) + DataHelper.GetDouble(proposalData["Funder"]),
                    timeTakenIn_AwaitingPayout__c = DataHelper.GetDouble(proposalData["Awaiting Payout"]),

                    timeTaIn_CreditPlusSalesSupportPlusRefer__c = DataHelper.GetDouble(proposalData["Active Proposals"]) + DataHelper.GetDouble(proposalData["New Proposals"]) + DataHelper.GetDouble(proposalData["Referred for Information"]) + DataHelper.GetDouble(proposalData["Credit Manager"]) + DataHelper.GetDouble(proposalData["Credit Committee"]) + DataHelper.GetDouble(proposalData["Funder"]),
                    
                };

                updateProposalRecordInSalesforce(salesforceClient, proposalDataForSF);
        ...
        private static void updateProposalRecordInSalesforce(SalesforceHttpClient salesforceClient, ProposalData proposalData)
        {
            ...
            Task.Run(async () =>
            {
                anchorWebServices = await salesforceClient.QueryAsync<ProposalData>(queryString);
            ...
            if (anchorWebServices.Records.Count > 0)
            {
                ...
                if (changed)
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        successResponse = await salesforceClient.UpsertExternalAsync("Proposal__c", "id", awsR.id, obj);
