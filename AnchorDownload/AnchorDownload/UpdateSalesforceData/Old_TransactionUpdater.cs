...
namespace UpdateSalesforceData
{
    ...
    {
        public static void UpdateAgreementTransactions(SqlConnection sqlConn, SqlConnection sqlConn1, SalesforceHttpClient salesforceClient)
        {
            ...
            ILookup<string, string> salesForceAgreementIds = GetSalesForceAgreementIds(salesforceClient);

            //var maxTransactionId = GetMaxTransactionsForSalesforceFromSQL(sqlConn);

            var agreementNumbersData = GetAgreementNumbersForSalesforceFromSQL(sqlConn);
            while (agreementNumbersData.Read())
            {
                ...
                var transactionsData = GetTransactionsForSalesforceFromSQL(sqlConn1, agreementNumber);
                ...
                while (transactionsData.Read())
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        var successResponse = await salesforceClient.UpsertExternalAsync("AgreementTransaction__c", "TransactionId__c", transactionId.ToString(), transactionObject);
        ...
        {
            return new SqlCommand(string.Concat("SELECT * FROM GetTransactionsForAgreement('", agreementNumber, "') ORDER BY TransCounter"), sqlConn){ CommandTimeout = 0 }.ExecuteReader();
        }
