...
namespace DocxFromTemplateGenerator
{
    public static class GenerateDocument
    {
        ...
        public static string Generate(string templateFile, string outputFilename,
            Dictionary<string, string> filedsWithData, Action<DocTemplateHandler, Dictionary<string,DataTable>> specificProcessing,
            Dictionary<string,DataTable> records)
        {
            DocTemplateHandler handler = new DocTemplateHandler(templateFile);
            handler.ReplaceValues(filedsWithData);

            if (specificProcessing != null && records != null)
            {
                specificProcessing(handler, records);
            }

            handler.Save(outputFilename);
            handler.Close();
